...
class APGD(Attack):
    def __init__(self, model, fwd_function=None, T=None,  surrogate='PCW', gamma=1.0, norm='Linf', eps=8/255, steps=10,
                 n_restarts=1, seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False):
        ...
        print('Auto-PGD attack with epsilon: ', eps, ' and loss: ', loss)
        if T > 0:
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        self.steps_2, self.steps_min, self.size_decr = max(int(0.22 * self.steps), 1), max(int(0.06 * self.steps), 1), max(
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                if self.forward_function is not None:
                    logits = self.forward_function(self.model, x_adv, self.T, self.surrogate, self.gamma)     # 1 forward pass (eot_iter = 1)
                else:
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        adv = x.clone()
        if self.forward_function is not None:
            outs = self.forward_function(self.model, x, self.T, self.surrogate, self.gamma)
        else:
            outs = self.model(x)
        acc = outs.max(1)[1] == y
