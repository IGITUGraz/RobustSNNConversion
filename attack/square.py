...
class Square(Attack):
    def __init__(self, model, fwd_function=None, T=None, surrogate='PCW', gamma=1.0, norm='Linf', eps=8/255,
                 n_queries=5000, n_restarts=1, p_init=.8, loss='margin', resc_schedule=True, seed=0, verbose=False):
        ...
        print('SquareAttack with epsilon: ', eps)
    ...
    def margin_and_loss(self, x, y):
        ...
        if self.forward_function is not None:
            logits = self.forward_function(self.model, x, self.T, self.surrogate, self.gamma)
        else:
            logits = self.model(x)
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    if self.verbose and ind_succ.numel() != 0 and (i_iter % 500 == 0):
    ...
    def perturb(self, x, y=None):
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    if self.forward_function is not None:
                        output = self.forward_function(self.model, x, self.T, self.surrogate, self.gamma)
                    else:
                        output = self.model(x)
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                if self.forward_function is not None:
                    output_curr = self.forward_function(self.model, adv_curr, self.T, self.surrogate, self.gamma)
                else:
                ...
                if self.verbose:
                    print('restart {} - robust accuracy: {:.2%}'.format(counter, acc.float().mean()),
